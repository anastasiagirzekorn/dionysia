...
function Popup({item,onClose}) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:100,background:"rgba(10,4,8,0.85)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div onClick={e=>e.stopPropagation()} style={{background:BG,border:"1px solid rgba(196,154,90,0.35)",borderRadius:16,padding:"28px 26px",maxWidth:480,width:"100%",position:"relative"}}>
        <button onClick={onClose} style={{position:"absolute",top:14,right:16,background:"none",border:"none",color:ACCENT,cursor:"pointer",fontSize:22,lineHeight:1}}>x</button>
...
function Card({route,label,onAsk}) {
      ...
      <div style={{width:"100%",maxWidth:700,flex:1,padding:"0 16px 16px",zIndex:1}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:12,marginBottom:28}}>
              {COUNTRIES.map(c=>(
                <button key={c.code} onClick={()=>{setCountry(c);setTab("famous");setView("country");}}
                  style={{background:CARD,border:"1px solid "+B,borderRadius:12,padding:"18px 12px",cursor:"pointer",textAlign:"center",color:TEXT,transition:"all 0.2s"}}
                  onMouseOver={e=>{e.currentTarget.style.background=CARD2;e.currentTarget.style.borderColor=B2;}}
                  onMouseOut={e=>{e.currentTarget.style.background=CARD;e.currentTarget.style.borderColor=B;}}
                >
        ...
        {view==="country"&&country&&(
          <div>
            ...
            {shown.map((r,i)=><Card key={i} route={r} label={t.ask} onAsk={send}/>)}
